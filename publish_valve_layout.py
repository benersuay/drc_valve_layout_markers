...
def marker_publisher():
    ...
    pipes = MarkerArray()
    leftPipe = pipe_factory(Marker(), 0, -0.6096)
    pipes.markers.append(leftPipe)

    midPipe = pipe_factory(Marker(), 1, 0.0)
    pipes.markers.append(midPipe)

    rightPipe = pipe_factory(Marker(), 2, 0.6096)
    pipes.markers.append(rightPipe)
    

    handles = MarkerArray()
    leftHandle = handle_factory(Marker(), 3, 0.3302, 0.6096, "lever")
